...
pub async fn process_with_qwen_cli(
    ...
    let system_prompt = "Ты - инструмент генерации контента и профессиональный маркетолог. Отвечай ТОЛЬКО текстом, который нужно скопировать и вставить. НЕ добавляй никаких поясняющих слов, комментариев, мыслей или системных сообщений. НЕ говори 'Ответ:', 'Результат:' или что-либо подобное. Просто предоставь запрашиваемый контент.";

    let user_prompt = format!("Категория: {}. Задача перефразировать, улучшить и дополнить текст для объявления на доске объявлений авито: {}", category, input_text);
