...
pub async fn process_with_qwen_cli(
    pool: Pool<Postgres>,
    ...
    let input_text = task
        ...
        .get("input_text")
        .and_then(|v| v.as_str())
        .or_else(|| task.request_data.parameters.get("title").and_then(|v| v.as_str()))
        .or_else(|| task.request_data.parameters.get("description").and_then(|v| v.as_str()))
        .ok_or("Input text not found in task parameters. Expected 'input_text', 'title', or 'description' field.")?
        .to_string();
    ...
    println!("Processing with qwen-cli: {}, category: {}", input_text, category);
    ...
    let system_prompt = "Ты - инструмент генерации контента. Отвечай ТОЛЬКО текстом, который нужно скопировать и вставить. НЕ добавляй никаких поясняющих слов, комментариев, мыслей или системных сообщений. НЕ говори 'Ответ:', 'Результат:' или что-либо подобное. Просто предоставь запрашиваемый контент.";

    let user_prompt = format!("Категория: {}. Задача перефразировать и сделать улучшенную версию текста: {}", category, input_text);
    
    // Combine system and user prompts
    let contextual_prompt = format!("{}\n\nПользовательский запрос: {}", system_prompt, user_prompt);
...
pub async fn oai_qwen_cli_processing(
    pool: Pool<Postgres>,
    ...
    loop {
        ...
        if needs_to_restart {
            ...
            // The actual processing logic is in the process_with_qwen_cli function
            return Ok(());
        }
    }
